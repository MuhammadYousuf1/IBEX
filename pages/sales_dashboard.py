...
if _sd_cached is not None and getattr(_sd_cached, '_SD_LOADED', False):
    ...
    # ------------------------------------------------------------------------
    # Workbook cache. Parsing SALES UPDATE.xlsx is expensive (openpyxl walks
    # the whole sheet on every call) and the dashboard needs the same sheets
    # many times while building its KPI frames, so each sheet is parsed once
    # per process and a *copy* is handed out on every request. Callers are
    # therefore free to mutate what they receive without corrupting the cache.
    # ------------------------------------------------------------------------
    _SHEET_CACHE = {}

    def _read_sheet(sheet_name, header=0):
        """Return a fresh copy of a workbook sheet, parsed at most once.

        ``header`` must match pandas' ``read_excel`` contract (int, list of int
        or None); its default of 0 is the same heading row the dashboard used
        to read before the cache was introduced.
        """
        key = (sheet_name, header)
        if key not in _SHEET_CACHE:
            _SHEET_CACHE[key] = pd.read_excel(DATA_PATH, sheet_name=sheet_name,
                                              header=header)
    ...
    def _load_df():
        try:
            # The workbook's first sheet is ActivationSheet, so read it by name
            # here to share the cached parse with the other loaders below.
            df = _read_sheet('ActivationSheet')
        except Exception:
            return pd.DataFrame()
        if df.empty:
            return df
    ...
    def _load_accessories_data():
        ...
        try:
            df = _read_sheet('SaledetailSheet')
        except Exception:
    ...
    def _load_storeperf_data():
        ...
        """
        try:
            raw = _read_sheet('Storeperf', header=None)
        except Exception:
