...
# Keeps an unreachable Supabase host from stalling startup (supabase-py
# defaults to a 120s postgrest timeout).
...
def get_supabase():
    ...
    A bounded postgrest timeout is requested so an unreachable Supabase host
    cannot stall app startup; the plain client is used if the installed
    supabase-py build does not expose ClientOptions.
    """
    ...
    try:
        ...
        _client = create_client(SUPABASE_URL, api_key, options=options)
    except Exception:
