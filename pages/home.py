...
# Live counts from SALES UPDATE.xlsx. The shared dropdown option lists already
# contain an extra 'All' entry, which is not a real store / DM / market.
STORE_COUNT = max(len(sd.STORE_OPTIONS) - 1, 0)
DM_COUNT = max(len(sd.DM_OPTIONS) - 1, 0)
MARKET_COUNT = max(len(sd.MARKETID_OPTIONS) - 1, 0)
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                html.P('Your centralized command center for sales performance, report downloads, and store deposit & expense tracking.',
            ...
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Div([
                                html.H2(f'{len(PAGES)}', className='fw-bold', style={'color': '#6366f1'}),
                                html.P('Active Dashboards', style={'color': '#94a3b8'})
                            ], className='text-center')
                        ], xs=6, md=3),
                        dbc.Col([
                            html.Div([
                                html.H2(f'{STORE_COUNT}', className='fw-bold', style={'color': '#10b981'}),
                                html.P('Store Locations', style={'color': '#94a3b8'})
                            ], className='text-center')
                        ], xs=6, md=3),
                        dbc.Col([
                            html.Div([
                                html.H2(f'{DM_COUNT}', className='fw-bold', style={'color': '#f59e0b'}),
                                html.P('Active Employees', style={'color': '#94a3b8'})
                            ], className='text-center')
                        ], xs=6, md=3),
                        dbc.Col([
                            html.Div([
                                html.H2(f'{MARKET_COUNT}', className='fw-bold', style={'color': '#06b6d4'}),
