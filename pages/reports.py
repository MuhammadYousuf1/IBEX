...
# Workbook sheet cache (shared by every reporter helper below).
_REPORT_CACHE = {}
...
def load_report(sheet_name):
    """Return a copy of one report sheet, parsed at most once per process.

    The preview is rebuilt on every filter change, so re-reading the workbook
    each time made the page feel sluggish. Failures are deliberately *not*
    cached, so a transient file problem recovers on the next attempt.
    """
    if sheet_name not in _REPORT_CACHE:
        try:
            _REPORT_CACHE[sheet_name] = pd.read_excel(sd.DATA_PATH,
                                                      sheet_name=sheet_name)
        except Exception:
            return pd.DataFrame()
    return _REPORT_CACHE[sheet_name].copy()
